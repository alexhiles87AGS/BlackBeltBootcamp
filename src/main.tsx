...
function App() {
  ...
  const activeTitle = page === 'session' ? 'Complete Workout' : visibleNav.find(n=>n.page===page)?.label || 'Dashboard';

  return <div className="appShell">
    <header className="topbar">
      <button className="iconButton" onClick={()=>setDrawerOpen(true)} aria-label="Open menu"><Menu size={24}/></button>
      <div className="topBrand"><Shield size={22}/><div><b>BlackBeltBootcamp</b><span>Training OS V2.2</span></div></div>
    ...
    <aside className={`drawer ${drawerOpen ? 'open' : ''}`}>
      <div className="drawerHead"><div className="brand"><Shield className="brandIcon"/><div><h1>BlackBeltBootcamp</h1><span>Training OS V2.2</span></div></div><button className="iconButton" onClick={()=>setDrawerOpen(false)}><X size={20}/></button></div>
...
function SessionWorkout({session,setSession,exercises,logs,setLogs,events,setEvents,onPlay}:{session:CalendarEvent|null; setSession:(s:CalendarEvent|null)=>void; exercises:Exercise[]; logs:WorkoutLog[]; setLogs:(l:WorkoutLog[])=>void; events:CalendarEvent[]; setEvents:(e:CalendarEvent[])=>void; onPlay:(e:Exercise)=>void}){
  ...
  const picks = useMemo(()=>pickExercisesForSession(fallback, exercises), [fallback?.id, exercises.length]);
  const [date,setDate]=useState(fallback.date || todayISO());
...
function Importer({setExercises,reloadSupabase,exercises}:{setExercises:(e:Exercise[])=>void; reloadSupabase:()=>void; exercises:Exercise[]}) {
  const [loaded,setLoaded]=useState(false); const [status,setStatus]=useState('Ready to load the bundled V2.2 catalogue.'); const [busy,setBusy]=useState(false); const [missingOpen,setMissingOpen]=useState(false);
