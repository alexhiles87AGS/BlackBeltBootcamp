...
function App() {
  ...
  return <div className="app">
    <aside className="sidebar">
      <div className="brand"><Shield className="brandIcon"/><div><h1>BlackBeltBootcamp</h1><span>Training OS V2.1.2</span></div></div>
...
function FmaClasses({events,setEvents}:{events:CalendarEvent[];setEvents:(e:CalendarEvent[])=>void}){
  ...
  const fmaEvents=events.filter(e=>e.type==='fma');
  function add(title:string){ setEvents([{id:crypto.randomUUID(),date:day,time,title:`FMA ${title}`,type:'fma',status:'planned'},...events]); }
  function setClassStatus(id:string,status:CalendarEvent['status']){ setEvents(events.map(e=>e.id===id?{...e,status}:e)); }
  return <section className="panel fmaPanel">
    <div className="row between"><div><h3>FMA Chester Classes</h3><p className="muted">Add James&apos;s class as a single class session. It does not create an individual exercise log.</p></div><span className="tag">Class session only</span></div>
    <div className="fmaControls"><label><span>Day</span><select value={day} onChange={e=>setDay(e.target.value)}>{['Mon','Tue','Wed','Thu','Fri','Sat','Sun'].map(d=><option key={d}>{d}</option>)}</select></label><label><span>Time</span><input type="time" value={time} onChange={e=>setTime(e.target.value)}/></label></div>
    <div className="grid three">{classes.map(c=><button className="sessionCard classSession" key={c} onClick={()=>add(c)}><Shield/><h4>{c}</h4><p className="muted">Add {day} · {time}</p></button>)}</div>
    <section className="subPanel"><div className="row between"><h4>Planned FMA class sessions</h4><span className="muted">No exercise sets/reps required</span></div>{fmaEvents.length===0 ? <p className="muted">No FMA classes added yet.</p> : fmaEvents.map(e=><div className="preview classPreview" key={e.id}><b>{e.title}</b><span>{e.date} · {e.time || 'Time TBC'} · {e.status}</span><div className="row actions compactActions"><button className="miniBtn" onClick={()=>setClassStatus(e.id,'completed')}>Attended</button><button className="miniBtn" onClick={()=>setClassStatus(e.id,'missed')}>Missed</button></div></div>)}</section>
  </section>
}
